...
async function bootFromUrl(url, options = {}) {
  url = url.replace(/\.part\.\d{3,}$/, "");
  const name = options.name || getPathLeaf(url);
  const bytes = await fetchSplitOrDirect(url, options.fetchInit);
  return bootRomData(name, bytes, options);
}

async function fetchSplitOrDirect(url, fetchInit) {
  const direct = await fetch(url, fetchInit).catch(() => null);
  if (direct && direct.ok) {
    return new Uint8Array(await direct.arrayBuffer());
  }
  const partUrl = (i) => `${url}.part.${String(i).padStart(3, "0")}`;
  const sizes = [];
  for (let i = 0; ; i++) {
    const h = await fetch(partUrl(i), { method: "HEAD" }).catch(() => null);
    if (!h || !h.ok) break;
    const len = Number(h.headers.get("content-length"));
    if (!Number.isFinite(len)) { sizes.length = 0; break; }
    sizes.push(len);
  }
  if (sizes.length === 0) {
    const status = direct ? `${direct.status} ${direct.statusText}` : "network error";
    throw new Error(`Failed to fetch ${url}: ${status} and no .part.000`);
  }
  const total = sizes.reduce((a, b) => a + b, 0);
  const out = new Uint8Array(total);
  let off = 0;
  for (let i = 0; i < sizes.length; i++) {
    const r = await fetch(partUrl(i), fetchInit);
    if (!r.ok) throw new Error(`Part ${i} failed: ${r.status}`);
